...
import { ArrowRight, Eye, EyeOff, Mail, Loader2, Sparkles, User, Lock, CheckCircle } from "lucide-react"
...
export default function RegisterPage() {
  ...
  const passwordCriteria = [
    ...
    { label: "One uppercase letter", met: /[A-Z]/.test(formValues.password) },
    { label: "One number", met: /\d/.test(formValues.password) },
    { label: "One special character", met: /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(formValues.password) },
  ]
  ...
  const handleRegister = async () => {
    ...
    try {
      await register(formValues.email.trim(), formValues.password)
      router.push("/verify-email")
    } catch (error) {
      if (error instanceof ApiError && error.status === 409) {
        setStatusMessage({ type: "error", message: "Email already exists." })
      } else {
        setStatusMessage({ type: "error", message: getApiErrorMessage(error, "Registration failed.") })
      }
    } finally {
              ...
              <div className="h-1.5 w-full bg-border/30 rounded-full overflow-hidden">
                <motion.div
                  ...
                  animate={{
                    ...
                    backgroundColor:
                      passwordStrength <= 2 ? "#ef4444" : passwordStrength === 3 ? "#eab308" : "#22c55e",
                  }}
                  transition={{ duration: 0.3 }}
                />
              </div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                {passwordCriteria.map((criterion) => (
                  <div key={criterion.label} className="flex items-center gap-1.5 text-xs">
                    <span className={criterion.met ? "text-green-500" : "text-muted"}>
                      {criterion.met ? "✓" : "○"}
                    </span>
                    <span className={criterion.met ? "text-foreground" : "text-muted"}>
                      {criterion.label}
                    </span>
                  </div>
                ))}
